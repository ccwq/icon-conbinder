...
export function readEnvFloat(name, fallback, min, max, env) {
  const value = env && Object.prototype.hasOwnProperty.call(env, name) ? env[name] : undefined;
  if (value === undefined || value === '') return fallback;
...
export function readEnvOneOf(name, options, fallback, env) {
...
export const SHAPE_OPTIONS = Object.entries(SHAPES).map(([value, shape]) => ({
  value,
  label: SHAPE_LABELS[value] || value,
  path: shape.path,
}));

export const ANTI_ALIAS_SCALES = [1, 2, 4];
export const RESIZE_STRATEGIES = ['smooth-high', 'pixelated', 'step-down', 'sharp-lanczos3'];
export const MAX_RENDER_DIMENSION = 4096;

export function parseMarginParam(raw, iconSize) {
...
export function parseState(query, env = (typeof process !== 'undefined' && process.env ? process.env : {})) {
  ...
  const defaults = {
    ...
    lineJoin: readEnvOneOf('ICON_PARAM_LINE_JOIN', ['round', 'miter', 'bevel'], 'round', env),
    borderColor: /^#[0-9a-fA-F]{6}$/.test(borderColorDefault) ? borderColorDefault : '#ef4444',
    bgColor: /^#[0-9a-fA-F]{6}$/.test(bgColorDefault) ? bgColorDefault : '#ffffff',
    ...
    contourCornerSoftness: readEnvFloat('ICON_PARAM_CONTOUR_CORNER_SOFTNESS', 0.12, 0, 1, env),
    ...
    exportStrategy: readEnvOneOf('ICON_PARAM_EXPORT_STRATEGY', ['center', 'bottom'], 'center', env),
    antiAliasScale: readEnvInt('ICON_PARAM_ANTI_ALIAS_SCALE', 1, 1, 4, env),
    resizeStrategy: readEnvOneOf('ICON_PARAM_RESIZE_STRATEGY', RESIZE_STRATEGIES, 'smooth-high', env),
  };
  ...
  return {
    ...
    iconSize,
    imageScale: clamp(
      query.imageScale === undefined ? defaults.imageScale : parseFloatOrFallback(query.imageScale, defaults.imageScale),
      0.01, 10
    ),
    imageOffsetY: clamp(
      query.imageOffsetY === undefined ? defaults.imageOffsetY : parseFloatOrFallback(query.imageOffsetY, defaults.imageOffsetY),
      -50, 50
    ),
    borderWidth: clamp(
      query.borderWidth === undefined ? defaults.borderWidth : parseFloatOrFallback(query.borderWidth, defaults.borderWidth),
      0, 20
    ),
    lineJoin: oneOf(query.lineJoin, ['round', 'miter', 'bevel'], defaults.lineJoin),
    borderColor: /^#[0-9a-fA-F]{6}$/.test(query.borderColor) ? query.borderColor : defaults.borderColor,
    bgColor: /^#[0-9a-fA-F]{6}$/.test(query.bgColor) ? query.bgColor : defaults.bgColor,
    contourEnhance: bool(query.contourEnhance, defaults.contourEnhance),
    contourOuterGlow: clamp(
      query.contourOuterGlow === undefined ? defaults.contourOuterGlow : parseFloatOrFallback(query.contourOuterGlow, defaults.contourOuterGlow),
      0, 20
    ),
    contourOuterWidth: clamp(
      query.contourOuterWidth === undefined ? defaults.contourOuterWidth : parseFloatOrFallback(query.contourOuterWidth, defaults.contourOuterWidth),
      0, 20
    ),
    contourMainWidth: clamp(
      query.contourMainWidth === undefined ? defaults.contourMainWidth : parseFloatOrFallback(query.contourMainWidth, defaults.contourMainWidth),
      0, 20
    ),
    contourInnerWidth: clamp(
      query.contourInnerWidth === undefined ? defaults.contourInnerWidth : parseFloatOrFallback(query.contourInnerWidth, defaults.contourInnerWidth),
      0, 20
    ),
    contourCornerSoftness: clamp(
      query.contourCornerSoftness === undefined ? defaults.contourCornerSoftness : parseFloatOrFallback(query.contourCornerSoftness, defaults.contourCornerSoftness),
      0, 1
    ),
    enableShadow: bool(query.enableShadow, defaults.enableShadow),
    shadowBlur: clamp(
      query.shadowBlur === undefined ? defaults.shadowBlur : parseFloatOrFallback(query.shadowBlur, defaults.shadowBlur),
      0, 50
    ),
    shadowOffsetY: clamp(
      query.shadowOffsetY === undefined ? defaults.shadowOffsetY : parseFloatOrFallback(query.shadowOffsetY, defaults.shadowOffsetY),
      -20, 20
    ),
    exportSquare: bool(query.exportSquare, defaults.exportSquare),
    exportStrategy: oneOf(query.exportStrategy, ['center', 'bottom'], defaults.exportStrategy),
...
export function getLayout(state) {
  ...
  const contourPad = state.contourEnhance ? Math.ceil(state.contourOuterWidth * 0.35 + state.contourOuterGlow * 0.75) : 0;
  const shadowSpread = state.enableShadow ? Math.max(0, Math.ceil(state.shadowBlur * 1.5)) : 0;
...
export function getRenderSize(state, layout = getLayout(state)) {
  return {
...
export function assertRenderSize(state, layout = getLayout(state)) {
  const renderSize = getRenderSize(state, layout);
  if (renderSize.width > MAX_RENDER_DIMENSION || renderSize.height > MAX_RENDER_DIMENSION) {
    throw new Error(
...
export function getInnerCircle(shapeKey, width, height) {
  switch (shapeKey) {
    case 'pin': {
      const r = width * 0.46;
...
export function resolveImageReference(rawImage, options = {}) {
  const { imageEnableBase64 = false, imageUrlPrefix = '', imageUrlPrefixOnly = false } = options;
  ...
  if (image.startsWith('data:')) {
    if (!imageEnableBase64) {
      throw clientError('IMAGE_BASE64_DISABLED', 'image 传入了 data:image/*;base64,...，但 IMAGE_ENABLE_BASE64 未开启');
    }
    if (!isDataImageUrl(image)) {
      throw clientError('IMAGE_BASE64_INVALID', 'image 必须是 data:image/*;base64,... 形式');
    }
    return { kind: 'data', value: image };
  }

  if (image.startsWith('//')) {
    throw clientError('IMAGE_URL_INVALID', 'image 不能以 // 开头，请使用 http(s):// 或相对路径');
  }
  ...
  if (isHttpUrl(image)) {
    ...
    if (!normalizedPrefix) {
      throw clientError('IMAGE_URL_PREFIX_REQUIRED', 'image 是相对路径时必须配置 IMAGE_URL_PREFIX');
    }
  ...
  if (imageUrlPrefixOnly) {
    if (!normalizedPrefix) {
      throw clientError('IMAGE_URL_PREFIX_REQUIRED', '启用 IMAGE_URL_PREFIX_ONLY 时必须配置 IMAGE_URL_PREFIX');
    }
    if (!resolvedUrl.startsWith(normalizedPrefix)) {
      throw clientError('IMAGE_URL_PREFIX_MISMATCH', 'image 解析后的 URL 必须命中 IMAGE_URL_PREFIX');
    }
  }
